...
public class LazyViewPager extends ViewGroup {
	...
	public interface OnPageChangeListener {
		...
		/**
		 * This method will be invoked when a news page becomes selected. Animation is not necessarily complete.
		 * 
		 * @param position
		 *            Position index of the news selected page.
		 */
	...
	public void setAdapter(PagerAdapter adapter) {
		...
		if (mAdapter != null) {
			...
			if (mRestoredCurItem >= 0) {
				...
				setCurrentItemInternal(mRestoredCurItem, false, false);
				mRestoredCurItem = -1;
	 ...
	 *            Item index to select
	 * @param smoothScroll
	 *            True to smoothly scroll to the news item, false to transition immediately
	 */
	...
	void setCurrentItemInternal(int item, boolean smoothScroll, boolean always) {
		setCurrentItemInternal(item, false, always, 0);
	}
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (mAdapter != null) {
			...
			setCurrentItemInternal(ss.position, false, false);
		} else {
	...
	public void endFakeDrag() {
		...
		if ((Math.abs(initialVelocity) > mMinimumVelocity) || Math.abs(mInitialMotionX - mLastMotionX) >= (getWidth() / 3)) {
			if (mLastMotionX > mInitialMotionX) {
				setCurrentItemInternal(mCurItem - 1, false, false);
			} else {
				setCurrentItemInternal(mCurItem + 1, false, false);
			}
		} else {
