...
/**
 * 
 * Title:  ActionSheetDialog<br>
 * Description: TODO  点击拍照后弹出的Dialog选项窗口<br>
 * Depend : TODO 
 * @author xuzhuchao
 * @ModifiedDate 
 * @since JDK 1.7
...
public class ActionSheetDialog {
	...
	public ActionSheetDialog builder(){
		View view = LayoutInflater.from(context).inflate(R.layout.wz_toast_view_actionsheet, null);
	...
	private void setSheetItems() {
		...
		for (int i = 1; i <= size; i++) {
			...
			if (size == 1) {
				if (showTitle) {
					textView.setBackgroundResource(R.drawable.wz_actionsheet_bottom_selector);
				} else {
					textView.setBackgroundResource(R.drawable.wz_actionsheet_single_selector);
				}
			} else {
				if (showTitle) {
					if (i >= 1 && i < size) {
						textView.setBackgroundResource(R.drawable.wz_actionsheet_middle_selector);
					} else {
						textView.setBackgroundResource(R.drawable.wz_actionsheet_bottom_selector);
					}
				} else {
					if (i == 1) {
						textView.setBackgroundResource(R.drawable.wz_actionsheet_top_selector);
					} else if (i < size) {
						textView.setBackgroundResource(R.drawable.wz_actionsheet_middle_selector);
					} else {
						textView.setBackgroundResource(R.drawable.wz_actionsheet_bottom_selector);
	...
	public enum SheetItemColor {
		Orange("#ff6900"), Red("#FD4A2E"),Grey("#747474"),Pink("#fc6a68");
